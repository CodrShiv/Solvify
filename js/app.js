...
// Populating all the Select Options and updating Given Options accordingly
...
solveBtn.onclick = () => {
  ...
  inputs.forEach((input) => {
    formula = formula.replace(new RegExp(`\\s${input.id}\\s`, "g"), input.value);
  });
  document.body.setAttribute("id", "solution-screen");
  let lhs = document.querySelector("#lhs").innerHTML;
  equation.parentElement.innerHTML = `<h2>${lhs} = ${eval(formula)} ${units[lhs]}</h2>`;
};
...
function validateAll() { return Array.from(document.querySelectorAll("input")).every((input) => !!input.value) };